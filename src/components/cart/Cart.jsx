...
const Cart = () => {
  ...
  // Handle order creation (updated for "self" not checking city or phone number)
  const handleContinue = async () => {
    try {
      ...
      if (!userId || orderItems.length === 0) {
        setNotification("User or cart data is missing.");
        return;
      }

      // Prepare order details based on whether "self" or "other" is selected
      const orderDetails = {
        userId,
        orderItems,
        ...
        orderFor,
        // Only set city and phoneNumber if "other" is selected
        city: orderFor === "other" ? city : userData?.city,
        phoneNumber: orderFor === "other" ? phoneNumber : userData?.phone_number,
      };
      ...
      if (response.ok) {
        ...
        setNotification(errorData.message || "Failed to place order.");
      }
    } catch (error) {
